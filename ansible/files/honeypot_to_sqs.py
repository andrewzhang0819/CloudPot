...
LOG_PATH = os.path.expanduser("/var/log/cowrie/cowrie.json") # gets the cowrie log path
...
def tail_file(path):
    """
    Wait for the file to exist, then yield new lines as they are written.
    Seeks to end of file on open to avoid replaying old logs on restart.
    """
...
def filter_failed_for_retry(original_batch, failed_messages):
    """
    Given the original batch entries and the list of SQS failure responses,
    return only the entries that failed due to a server-side error (retryable).
    Client/sender faults are not retried since resending won't fix them.
    """
    retryable_ids = {
        failure["Id"]
...
def send_batch_with_retry(sqs_client, batch, max_retries=3):
    """
    Send a batch to SQS and retry server-side failures up to max_retries times.
    Logs any messages that could not be sent after exhausting retries.
    """
    current_batch = batch
...
def main():
    ...
    for line in tail_file(LOG_PATH):
        ...
        try:
            ...
            message_body = json.dumps({"raw": line})
            # message_body = json.dumps({"raw": line, "sensor_id": SENSOR_ID})
