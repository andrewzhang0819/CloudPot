...
EC2_IP=$(terraform output -raw honeypot_ip)
SQS_URL=$(terraform output -raw sqs_queue_url)
 
sed -i "s|^EC2_IP=.*|EC2_IP=$EC2_IP|" variables.env
...
until ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 -i "$HOME/.ssh/id_rsa" -p 20022 ubuntu@"$EC2_IP" "exit" >/dev/null 2>&1; do
    echo "EC2 instance not ready yet... retrying in 10 seconds"
    sleep 10
done
echo "EC2 instance is ready... proceeding"
...
# Deploy local ELK stack
# docker-compose up -d

# Output Kibana link
echo "Deployment complete. Access your Kibana dashboard at: http://localhost:5601"
